	...
	public function process()
	{
		...
		{			
			...
			if ($bIsTagSearch)
			{
				if ($bIsModuleTagSearch)
				{
					$this->template()->setBreadcrumb(Phpfox::getPhrase('forum.threads_tagged_with') . ': ' . str_replace('-', ' ', $this->request()->get('req5')), $this->url()->makeUrl('forum.tag.module_group.item_' . $this->request()->get('item') . '.'  . $this->request()->get('req5')), true);
				}
				else 
				{
					$this->template()->setBreadcrumb(Phpfox::getPhrase('forum.tags'), $this->url()->makeUrl('forum.tag'))->setBreadcrumb(Phpfox::getPhrase('forum.threads_tagged_with') . ': ' . str_replace('-', ' ', $this->request()->get('req3')), $this->url()->makeUrl('forum.tag.'  . $this->request()->get('req3')), true);
				}
			}
			else 
			{
