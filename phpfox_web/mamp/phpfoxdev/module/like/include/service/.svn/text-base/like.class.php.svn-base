 ...
 * @version 		$Id: like.class.php 5469 2013-03-04 08:22:34Z Miguel_Espinoza $
 */
class Like_Service_Like extends Phpfox_Service 
{
	...
	public function getLikesForFeed($sType, $iItemId, $bIsLiked = false, $iLimit = 4, $bLoadCount = false)
	{
		...
		$aLikes = $this->database()->select('l.*, ' . Phpfox::getUserField() .', a.time_stamp as action_time_stamp')
			->from($this->_sTable, 'l')
	...
	public function hasBeenMarked($iActionType, $sTypeId, $iItemId, $iUserId = null)
	{
		...
		$iActionId = $this->database()->select('action_id')
			...
			->where('action_type_id = ' . (int)$iActionType .' AND item_type_id = "' . $oParse->clean($sTypeId) . '" AND item_id = ' . (int)$iItemId . ' AND user_id = ' . (int)$iUserId)
			->execute('getSlaveField');
	...
	public function getActionsFor($sItemTypeId, $iItemId)
	{
		...
		else if ($sItemTypeId == 'custom-relation')
		{
		    $sItemTypeId = 'custom';
		}        
		if ($sItemTypeId == 'custom' && Phpfox::getParam('user.enable_relationship_status') != true)
		{
			return false;
		}
		...
		{
			return false;
			throw new Exception ('Not a valid module or without the getActions callback: ' . $aModule[0]);
		}
		...
		{
			if ( ($aAction['item_type_id'] == 'custom_relation' && $sItemTypeId == 'custom' ) || $aAction['item_type_id'] == $sItemTypeId || ($aModule[0] . '-' . $aAction['item_type_id']) == $sItemTypeId || ($aModule[0] == $aAction['item_type_id']))
			{
				$aThisAction = $aAction;
				break;
			}
		...
		{
			return false;
			throw new Exception ('Could not find specific action: ' . print_r($aModule, true) . ' sItemTypeId:' . $sItemTypeId);
		}
		if (!isset($aThisAction['phrase_in_past_tense']))
		{
			return false;
			throw new Exception ('Action does not have a phrase in past tense ("disliked") for ' . $aModule[0]);
		}
		
		if ($sItemTypeId == 'custom')
		{
		    $sItemTypeId = 'custom_relation';
		}
         ...
         * 
         */
        public function didILike($sType, $iItemId)
        {
