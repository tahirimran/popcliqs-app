		...
		$this->template()->setTitle($aVideo['title'])
			...
			if (!$aVideo['is_stream'])
			{
				...
				$this->template()->setHeader(array(
						...
						'<script type="text/javascript">var iPlayVideo' . $aVideo['video_id'] . ' = false; $Behavior.playVideo = function() { ' . $sJsMobile . ' if (iPlayVideo' . $aVideo['video_id'] . ') { return; } iPlayVideo' . $aVideo['video_id'] . ' = true; $Core.player.load({id: \'js_video_player\', auto: true, type: \'video\', play: \'' . $sVideoPath . '\'}); }</script>'
					)
				);
			}
			...
			if (Phpfox::getParam('video.vidly_support') && $aVideo['in_process'] == '1')
			{
